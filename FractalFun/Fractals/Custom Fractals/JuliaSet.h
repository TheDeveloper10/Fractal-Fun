...
class JuliaSet : public FractalCore {
public:
	size_t max_iterations = 64;
	double fs_w_min = -1.5, fs_w_max = 1.5;
	double fs_h_min = -1.5, fs_h_max = 1.5;
...
public:
	void draw() {
		...
		for (size_t i = 0; i < THREADS_COUNT; ++i) {
			//threads.draw_part(k);
			threads.push_back(
				std::thread(&JuliaSet::draw_part, this, i * k, (i + 1) * k)
			);
		}
