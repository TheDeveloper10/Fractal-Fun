...
class BurningShipFractal_AVX2 : public FractalCore {
public:
	size_t max_iterations = 64;
	double fs_w_min = -1.0, fs_w_max = 1.0;
	double fs_h_min = -1.0, fs_h_max = 1.0;
...
private:
	void draw_part(size_t colStart, size_t colEnd) {
		...
		const __m256d _two = _mm256_set1_pd(2.0),
						...
						_sign_mask = _mm256_castsi256_pd(
										_mm256_set1_epi64x(
												~((int64_t)1 << 63)
										));
		...
		for (size_t j = 0; j < m_image->getHeight(); ++j) {
			...
			// _ci = mapped y coord
			_ci = _mm256_set1_pd(map(j, 0, m_image->getHeight(), fs_h_min, fs_h_max));
			...
			_loop_start:
				...
				// mask the bit responsible for the sign
				_zr = _mm256_and_pd(_zr, _sign_mask);
				_zi = _mm256_and_pd(_zi, _sign_mask);
...
public:
	void draw() {
		...
		for (size_t i = 0; i < THREADS_COUNT; ++i) {
			//threads.draw_part(k);
			threads.push_back(
				std::thread(&BurningShipFractal_AVX2::draw_part, this, i * k, (i + 1) * k)
			);
		}
